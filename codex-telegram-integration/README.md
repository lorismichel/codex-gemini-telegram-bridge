...
Single-project bridge for controlling a local Codex CLI session from Telegram.

## How It Works

- `npm run start` launches the `manager`.
- The `manager` polls the Telegram Bot API.
- When a message arrives, the `worker` runs `codex` on demand.
- The `worker` is forked internally by the manager, so `npm run worker` is usually not needed.

Codex does not need to be running in advance.
...
- `codexCmd`: `codex`
- `codexArgs`: `["--search", "--dangerously-bypass-approvals-and-sandbox"]`
- `allowedCommands`: `status`, `diff`, `log`
- `allowAnyCommand`: `true`
- `allowFirstChatRegistration`: `true`
...
Use an override like this only if you want to disable unrestricted `/run` execution or remove Codex approval and sandbox bypass.
...
- The default Codex configuration enables `--dangerously-bypass-approvals-and-sandbox`, so Codex runs in a high-trust mode unless you explicitly remove that flag.

## Wrapper Script

`scripts/start-codex-with-telegram.sh` is not a replacement for `npm run start`.

It is a helper script that:

1. Starts the manager in the background if it is not already running.
2. Then launches the `codex` CLI itself.
